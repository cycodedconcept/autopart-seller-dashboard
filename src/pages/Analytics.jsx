...
import {
  ...
  Eye, Pencil, Trash2, TrendingUp, TrendingDown,
} from 'lucide-react'
import {
...
function KpiCard({ icon: Icon, label, value, change, up }) {
  return (
    <div className="an-kpi-card">
      <div className="an-kpi-top">
        <div className="an-kpi-icon"><Icon size={15} /></div>
        <span className="an-kpi-label">{label}</span>
      </div>
      <div className="an-kpi-value">{value}</div>
      <div className={`an-kpi-change ${up ? 'up' : 'dn'}`}>
        <span className="an-kpi-pct">{change}</span>
        <span className="an-kpi-last">last month</span>
      </div>
    </div>
  )
}
...
const CITY_MARKERS = [
  { city: 'Lagos',         amount: 23231, x: 80,  y: 180, color: '#FF6B00' },
  { city: 'Abuja',         amount: 12091, x: 320, y: 120, color: '#EF4444' },
  { city: 'Port Harcourt', amount: 17321, x: 560, y: 150, color: '#F59E0B' },
  { city: 'Kano',          amount: 19534, x: 480, y: 220, color: '#10B981' },
...
const DONUT_COLORS = ['#10B981', '#EF4444', '#F59E0B', '#6366F1']
  ...
  return (
    <div className="analytics-page">
      <div className="an-kpi-grid">
        <KpiCard icon={Package} label="No. of Products" value="5,536" change="+15.5%" up />
        <KpiCard icon={Users} label="Reg. Agents" value="746" change="-02.8%" up={false} />
        <KpiCard icon={DollarSign} label="Total Revenue" value="₦2,748" change="+21.6%" up />
        <KpiCard icon={Target} label="Target This Month" value="435" change="-09.4%" up={false} />
      </div>
      ...
      <div className="an-row-2">
        <div className="card an-chart-card">
          ...
          <div className="an-line-legend">
            ...
            <span className="an-legend-dot" style={{ background: '#10B981', marginLeft: 12 }} />
            ...
            <BarChart data={totalRevenueBarData} barSize={22} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
              ...
              <Bar dataKey="a" stackId="rev" name="Sold">
                {totalRevenueBarData.map((e, i) => (
                  <Cell key={i} fill={e.active ? '#FF6B00' : 'rgba(249,115,22,0.2)'} />
                ))}
              </Bar>

              <Bar dataKey="b" stackId="rev" name="Rented">
                {totalRevenueBarData.map((e, i) => (
                  <Cell key={i} fill={e.active ? '#F59E0B' : 'rgba(245,158,11,0.2)'} />
                ))}
              </Bar>
