...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault()
    const action = await dispatch(sellerLogin({ email, password }))
    if (!sellerLogin.fulfilled.match(action)) return
    ...
    const status = action.payload.sellerProfile?.verificationStatus
    navigate(status === 'approved' || status === 'verified' ? '/dashboard' : '/pending-verification', { replace: true })
  }
