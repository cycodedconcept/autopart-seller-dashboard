...
import {
  ...
  PanelLeftClose, Bell, LogOut, Scale, Moon
} from 'lucide-react'
import { useSelector, useDispatch } from 'react-redux'
import { logout } from '../features/authSlice'
import autoLogo from '../assets/Logo.png'
import { isEvening, timeGreeting } from '../utils/greeting'
...
const menuItems = [
  ...
  { icon: Scale, label: 'Disputes', to: '/disputes' },
  { icon: BarChart3, label: 'Analytics', to: '/analytics' },
  { icon: ArrowUpDown, label: 'Sales & payouts', to: '/sales' },
...
export default function Layout() {
  ...
  const displayName = user?.fullName || user?.name || 'Kamal Okelola'
  const displayAvatar = user?.avatar || 'https://picsum.photos/200/200?random=100'
  const GreetingIcon = isEvening(currentTime) ? Moon : Sun
        ...
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div className="greeting">
              <GreetingIcon size={20} style={{ color: '#FFA500' }} />
              <span className="greeting-text">{timeGreeting(currentTime)}, {displayName}!</span>
            </div>
          </div>
