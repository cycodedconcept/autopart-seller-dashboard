...
export default function StatCard({ 
  ...
  const iconContainer = () => {
    if (iconVariant === 'tile') {
      return {
        width: '32px', height: '32px', border: '1px solid #EEEEEE', borderRadius: '8px',
  ...
  return (
    <div className="card dashboard-stat-card" style={{ 
      padding: graph ? '18px 6px 18px 16px' : isDashboard ? '16px' : '20px 22px',
      display: 'flex', 
      flexDirection: graph ? 'row' : 'column', 
      gap: cardGap,
      position: 'relative',
      overflow: 'hidden',
      borderRadius: isDashboard ? '12px' : '18px',
      boxShadow: isDashboard ? 'none' : '0 1px 3px rgba(15,23,42,.03), 0 6px 18px rgba(15,23,42,.03)'
    }}>
      <div style={{ 
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={iconStyle}>
            <Icon size={18} color={iconColor} strokeWidth={isDashboard ? 1.5 : 2} aria-hidden="true" />
          </div>
          <span style={{ fontSize: titleSize || '16px', color: '#5F5F5F', fontWeight: 500, lineHeight: '20px', letterSpacing: '-0.2px' }}>{label}</span>
        </div>
      ...
      <div style={{ 
        ...
        {customChange || (showChange && change && (
          <div style={{ display: 'flex', alignItems: 'center', gap: isDashboard ? '6px' : '2px', marginTop: '-2px', flexWrap: isDashboard ? 'wrap' : 'nowrap' }}>
            <div style={{ 
              padding: '2px 8px',
              borderRadius: '4px',
